...
def tokenize(ar_text):
    ...
    tok_text=[]
    print flat_file_path + ".mada"
    with codecs.open(flat_file_path + ".mada","r","utf-8") as infile:
        sents =re.split('\n-+\nSENTENCE BREAK\n-+\n;;; SENTENCE', infile.read())
        print len(sents)
	for ii in sents:
	    toks = re.findall(r'stem:([^\s]*)', ii, re.M)
	    print (len(toks))
	    print ' '.join(toks)
            tok_text.append(' '.join(toks))
...
if __name__ == '__main__':
    ...
    csvobj,csvobj2=itertools.tee(csvobj)
    
    tweets=[]
    ind=0
    # Create separate file with documents to be sent to madamira at once
    for ii in csvobj:
        #print 'Doc %d'%ind

        ii[ind_tweet]=re.sub('\*[^\*]+\*','',ii[ind_tweet])
        ii[ind_tweet]=re.sub('\n','',ii[ind_tweet])
        tweets.append(ii[ind_tweet].strip())
        ind += 1

    #print '\n'.join(tweets).encode('ascii', 'xmlcharrefreplace')
    tokenized=tokenize('\n'.join(tweets))
    ...
    for ii in csvobj:
        ...
        if not ii[ind_relev] == 'R' or not len(ii) == len(header):
            #print 'Skipping %d'%ind
            continue
    ...
    csvobj=UnicodeWriter(open('%s/%s.%s'%(args.out_folder, os.path.basename(args.filename), args.combine_annotations),'w'),delimiter=',')
    
    print '%s/%s.%s'%(args.out_folder, os.path.basename(args.filename), args.combine_annotations)
	...
	csvobj.writerow([ ii, str(round(combine_method(dict_tweets[ii][0]))), \
    			  str(round(combine_method(dict_tweets[ii][1]))), \
    			  str(round(combine_method(dict_tweets[ii][2]))), \
    			  str(round(combine_method(dict_tweets[ii][3]))), \
    			  str(round(combine_method(dict_tweets[ii][4]))), \
    			  dict_tweets[ii][5] ])
    '''
    for ind,ii in enumerate(dict_tweets.iterkeys()):
