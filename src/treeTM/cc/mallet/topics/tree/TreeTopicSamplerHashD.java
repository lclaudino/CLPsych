...
public abstract class TreeTopicSamplerHashD extends TreeTopicSampler implements TreeTopicSamplerInterface{
	...
	public void resumeStates(InstanceList[] training, String statesFile) throws IOException{
		...
		// Leo: added UTF-8
		BufferedReader states = new BufferedReader(new InputStreamReader(statesdstream,"UTF8"));
		...
		for(int ll = 0; ll < training.length; ll++) {
			for (Instance instance : training[ll]) {
				...
				System.out.println("Leo: in resume, read doc name="+name);
				...
				for (int jj = 0; jj < original_tokens.getLength(); jj++) {
					...
					original_tokens.getObjectAtPosition(jj)).getBytes("utf-8") );
					...
					if(token != -1 && removed == -1) {
						...
						System.out.println("Leo: token: " + String.valueOf(token));
						System.out.println("Leo: removed: " + String.valueOf(removed));
						System.out.println("Leo: word: " + word);
				...
				if(count != -1) {
					...
					/*
					System.out.println("Leo: str.length" + String.valueOf(str.length));
					System.out.println("Leo: count" + String.valueOf(count));
					*/
